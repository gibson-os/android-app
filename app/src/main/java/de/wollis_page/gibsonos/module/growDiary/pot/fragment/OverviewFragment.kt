package de.wollis_page.gibsonos.module.growDiary.pot.fragment
...
class OverviewFragment: AbstractOverviewFragment() {
    ...
    override fun loadOverviewModel() {
        this.activity.runTask({
            val pot = PotTask.get(this.activity, this.fragmentsArguments["potId"].toString().toLong())
            val image = PotTask.image(this.activity, pot.id, this.view?.findViewById<ImageView>(R.id.image)?.width)
            val viewModel = this.viewModel

            this.activity.runOnUiThread {
                viewModel.addItem(TitleBuilder(pot.name))
                viewModel.addItem(HeadlineBuilder(pot.name))
                viewModel.addItem(ManufactureBuilder(pot.manufacture))
                viewModel.addItem(TextBuilder("${pot.liter} l", this.getString(R.string.grow_diary_pot_liter)))
    ...
    override fun actionOnClickListener() {
        this.activity.runTask({
            ActivityLauncherService.startActivity(
                ...
                mapOf(
                    "task" to "pot",
                    "id" to this.fragmentsArguments["potId"].toString().toLong(),
                ),
                this.formLauncher,
            )
        })
